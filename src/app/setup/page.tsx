...
export default function SetupPage() {
  ...
  useEffect(() => {
    fetch("/api/setup")
      ...
      .catch((e) => setMsg(`Setup ma’lumotlari yuklanmadi: ${e?.message ?? "unknown"}`));
  }, []);
  ...
  async function save() {
    ...
    try {
      ...
      if (!res.ok || !json?.ok) {
        const errorText = json?.error || text || "setup failed";
        const debugText = json?.debug ? ` | debug: ${JSON.stringify(json.debug)}` : "";
        setMsg(`Xato ${res.status}: ${errorText}${debugText}`);
        return;
      }
      ...
      setMsg(`Xato: ${e?.message ?? "setup failed"}`);
    } finally {
      setSaving(false);
    }
  }
      ...
      <div className="mx-auto max-w-5xl px-5 py-8 space-y-6">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-white/65 max-w-2xl">
              Birinchi ishga tushirish, brending, Telegram bot, kanal va admin egasini bitta joydan ulang.
            </p>
          </div>
          <Link href="/" className="rounded-full border border-white/15 px-4 py-2 text-sm hover:bg-white/10">
            Public
          </Link>
        </div>

        {needsSetup === true && (
          <div className="rounded-3xl border border-amber-400/30 bg-amber-400/10 p-4 text-amber-100">
            Hali admin yo‘q. Birinchi admin Telegram ID kiriting. SETUP_SECRET qo‘ygan bo‘lsangiz shu yerda kiriting.
          </div>
        )}

        {msg && <div className="rounded-2xl border border-white/10 bg-white/10 p-4 text-sm whitespace-pre-wrap">{msg}</div>}
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <section className="rounded-3xl border border-white/10 bg-white/[0.04] p-5 space-y-4">
            ...
            <Field label="Sayt URL" value={settings.site_url} onChange={(v) => set("site_url", v)} placeholder="https://tfcjelly.uz" />
          ...
          <section className="rounded-3xl border border-white/10 bg-white/[0.04] p-5 space-y-4">
            ...
            <Field label="SETUP_SECRET" value={setupSecret} onChange={setSetupSecret} placeholder="Railway Variables ichidagi SETUP_SECRET" />
          </section>
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Link href="/login" className="rounded-2xl border border-white/15 px-6 py-3 font-bold hover:bg-white/10">
            Telegram login
          </Link>
          <Link href="/admin" className="rounded-2xl border border-white/15 px-6 py-3 font-bold hover:bg-white/10">
            Admin panel
          </Link>
        </div>
      </div>
