...
export default function SeasonTeamsPage() {
  ...
  async function loadAll() {
    ...
    // season
    const s = await dbClient.from("seasons").select("id,title,created_at").eq("id", seasonId).single();
    if (s.error) {
    ...
    // teams
    const t = await dbClient.from("teams").select("id,name,logo_url").order("name", { ascending: true });
    if (t.error) {
  ...
  async function addExistingTeam() {
    ...
    const res = await dbClient.from("season_teams").insert({
      season_id: seasonId,
  ...
  async function removeTeam(seasonTeamId: string) {
    setMsg(null);
    const res = await dbClient.from("season_teams").delete().eq("id", seasonTeamId);
    if (res.error) {
  ...
  async function createTeamAndAdd() {
    ...
    // 2) add to season
    const add = await dbClient.from("season_teams").insert({ season_id: seasonId, team_id: teamId });
    if (add.error) {
