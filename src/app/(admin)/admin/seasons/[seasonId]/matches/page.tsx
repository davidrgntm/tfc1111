...
export default function SeasonMatchesAdminPage() {
  ...
  async function generateSchedule() {
    ...
    for (let i = 0; i < rows.length; i += chunkSize) {
      ...
      const ins = await dbClient.from("matches").insert(chunk);
      if (ins.error) {
  ...
  async function createMatchManual() {
    ...
    const checkRound = await dbClient
      .from("matches")
    ...
    const pairCheck = await dbClient
      .from("matches")
      .select("id,home_team_id,away_team_id")
      .eq("season_id", seasonId)
      .or(
  ...
  async function deleteMatch(matchId: string) {
    ...
    const del = await dbClient.from("matches").delete().eq("id", matchId);
    if (del.error) {
