...
export default function AdminMatchMediaPage() {
  ...
  function publicUrl(path: string) {
    return dbClient.storage.from("match-photos").getPublicUrl(path).data.publicUrl;
  }
  ...
  async function saveHighlight() {
    ...
    const { error } = await dbClient.from("match_media").upsert(
      {
        match_id: matchId,
  ...
  async function uploadFiles(files: FileList | null) {
    ...
    for (const file of Array.from(files)) {
      ...
      const up = await dbClient.storage.from("match-photos").upload(path, file, {
        cacheControl: "3600",
        upsert: false,
      });
      ...
      const ins = await dbClient.from("match_photos").insert({
        match_id: matchId,
        storage_path: path,
        caption: null,
      });
  ...
  async function updateCaption(id: string, caption: string) {
    setMsg(null);
    const { error } = await dbClient.from("match_photos").update({ caption }).eq("id", id);
  ...
  async function deletePhoto(row: PhotoRow) {
    ...
    const delStorage = await dbClient.storage.from("match-photos").remove([row.storage_path]);
    if (delStorage.error) return setMsg(`Storage delete xato: ${delStorage.error.message}`);

    const delRow = await dbClient.from("match_photos").delete().eq("id", row.id);
