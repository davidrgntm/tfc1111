...
export default function AdminSeasonTeamsPage() {
  ...
  async function load() {
    ...
    // all teams
    const teamsRes = await dbClient.from("teams").select("id,name,logo_url").order("name", { ascending: true });
    if (teamsRes.error) {
  ...
  async function addExistingTeam() {
    ...
    const ins = await dbClient.from("season_teams").insert({ season_id: seasonId, team_id: pickTeamId });
  ...
  async function createAndAddTeam() {
    ...
    // 2) link to season
    const linkRes = await dbClient.from("season_teams").insert({ season_id: seasonId, team_id: teamId });
    if (linkRes.error) {
