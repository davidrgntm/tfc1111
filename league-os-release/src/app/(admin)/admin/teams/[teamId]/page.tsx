...
export default function EditTeamPage() {
  ...
  async function load() {
    ...
    const res = await dbClient.from("teams").select("id,name,logo_url").eq("id", teamId).single();
  ...
  async function save() {
    ...
    const res = await dbClient.from("teams").update({ name: n }).eq("id", teamId);
  ...
  async function uploadLogo(file: File) {
    ...
    const up = await dbClient.storage.from("logos").upload(path, file, {
      upsert: true,
    ...
    const pub = dbClient.storage.from("logos").getPublicUrl(path);
    const publicUrl = pub.data.publicUrl;

    const upd = await dbClient.from("teams").update({ logo_url: publicUrl }).eq("id", teamId);
    if (upd.error) {
